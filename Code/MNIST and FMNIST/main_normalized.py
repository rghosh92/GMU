# -*- coding: utf-8 -*-
"""
Created on Sat Mar 28 23:40:48 2026
...
class Net_vanilla_CNN(nn.Module):
    def __init__(self,input_channels,layers,kernels,multiplier, epsilons = [0.0001,0.0001,0.0001],decay_regress=0,decay_errors = 0, classes = 10,use_bn = True,dropping=0,poly_order_init=5):
        ...
        if self.mode =='normal':
            self.conv1 = NormalizedUnfoldConv2d(1, self.m*32, 3, padding=1)
...
def test_network_corrupted(net, dataset_name, corruptions,atan_convert):
    ...
    for corruption in corruptions:
        ...
        labels_test = torch.from_numpy(labels_test)
        data_test = (torch.from_numpy(data_test)).float().squeeze().unsqueeze(1)/255.0
        print(data_test.max(),data_test.min())
        # if atan_convert>0:
        #     data_test = map_inf(data_test, atan_convert)
...
if __name__ == "__main__":
    ...
    net = Net_vanilla_NormalizedCNN_convert(input_channels,layers,kernels,gmu_epsilons,decay_regress,decay_errors,use_bn=use_bn,dropping=dropping)
    ...
    accor = test_network_corrupted(net, dataset_name, corruptions,atanh_convert)
