...
def run_linear_probe_halved(train_x, train_y, layer_name, num_classes=10, epochs=100, lr=0.01, batch_size=200):
    # Ensure inputs are PyTorch Tensors on GPU
    if not isinstance(train_x, torch.Tensor):
        train_x = torch.as_tensor(train_x, dtype=torch.float32).cuda()
        train_y = torch.as_tensor(train_y, dtype=torch.long).cuda()
    else:
        train_x, train_y = train_x.cuda(), train_y.cuda()

    # --- 50/50 Split of train_x and train_y ---
    half_size = train_x.shape[0] // 2
    
    # First half = actual train set
    tr_x, tr_y = train_x[:half_size], train_y[:half_size]
    # Second half = test set
    te_x, te_y = train_x[half_size:], train_y[half_size:]

    # Min-Max Normalization (Fit on 1st half, Transform both)
    eps = 1e-8
    x_min = tr_x.min(dim=0, keepdim=True).values
    x_max = tr_x.max(dim=0, keepdim=True).values

    tr_x = (tr_x - x_min) / (x_max - x_min + eps)
    te_x = (te_x - x_min) / (x_max - x_min + eps)

    g_cuda = torch.Generator(device='cuda')

    train_loader = DataLoader(TensorDataset(tr_x, tr_y), batch_size=batch_size, shuffle=True, generator=g_cuda)
    test_loader  = DataLoader(TensorDataset(te_x, te_y), batch_size=batch_size, shuffle=False, generator=g_cuda)

    in_features = tr_x.shape[1]
    probe = nn.Linear(in_features, num_classes).cuda()
    optimizer = torch.optim.Adam(probe.parameters(), lr=lr)
    loss_fn = nn.CrossEntropyLoss()
    ...
    print(f"[{layer_name}] (50/50 Train Split) Train Acc: {train_acc*100:.2f}% | Test Acc: {test_acc*100:.2f}%")
    return train_acc, test_acc
...
#     corruptions = ['general']
    ...
    if dataset_name == 'SVHN':
        ...
        handle_gmu = resnet18_gmu.conv1.register_forward_hook(get_gmu_matched_features(features_gmu, 'conv1'))        
        
        
        print("Extracting features for Normal ResNet18...")
        train_x_norm, train_y = extract_first_layer(resnet18, trainloader, features_normal)
        # test_x_norm, test_y   = extract_first_layer(resnet18, testloader, features_normal)
        
        print("Extracting features for GMU ResNet18...")
        train_x_gmu, _ = extract_first_layer(resnet18_gmu, trainloader, features_gmu)
        # print('done')
        
        # print(train_x_gmu.shape)
        # test_x_gmu, _  = extract_first_layer(resnet18_gmu, testloader, features_gmu)
        # 
        # Clean up hooks so they don't fire later
        handle_norm.remove()
        handle_gmu.remove()
        
        # 6. Run the Linear Probes (using your existing function)
        print("\n--- Probing First Layer ---")
        run_linear_probe_halved(train_x_norm, train_y, layer_name="ResNet18 Normal (conv1)")
        run_linear_probe_halved(train_x_gmu, train_y, layer_name="ResNet18 GMU (conv1)")
    ...
    if dataset_name == 'MNIST':
        ...
        net1.load_state_dict(torch.load('CNN_convert_3slice_1degree_MNIST_[3, 8]degree_300epochs_[25, 50]layers60000data.h5',weights_only=True))
        ...
        net.feat_net[2].register_forward_hook(get_features(features_normal, 'layer1'))
        net.feat_net[7].register_forward_hook(get_features(features_normal, 'layer2'))
        net.feat_net[11].register_forward_hook(get_features(features_normal, 'layer3'))
        
        # For Convert3 / GMU CNN:
        net1.srn1.register_forward_hook(get_features(features_gmu, 'layer1'))
        net1.feat_net[5].register_forward_hook(get_features(features_gmu, 'layer2'))
        net1.feat_net[9].register_forward_hook(get_features(features_gmu, 'layer3'))
        
        print("Extracting features for Normal CNN...")
        train_feat_norm, train_labels = extract_dataset_features(net, trainloader, features_normal)
        test_feat_norm, test_labels   = extract_dataset_features(net, testloader, features_normal)
        
        print("Extracting features for Convert3 CNN...")
        train_feat_conv, _ = extract_dataset_features(net1, trainloader, features_gmu)
        test_feat_conv, _  = extract_dataset_features(net1, testloader, features_gmu)
        
        
        for layer in ['layer1', 'layer2', 'layer3']:
            print(f"\n--- Probing {layer} ---")
            print("Normal CNN:")
            run_linear_probe(train_feat_norm[layer], train_labels, test_feat_norm[layer], test_labels, layer)
            print("Convert3 CNN (with SRN):")
            run_linear_probe(train_feat_conv[layer], train_labels, test_feat_conv[layer], test_labels, layer)
        ...
        accuracy = test_network(net1, testloader)
    ...
    if dataset_name == 'FashionMNIST':
        ...
        net1.load_state_dict(torch.load('Best_CNN_convert_FashionMNIST_[3, 8]degree_400epochs_[25, 50]layers60000data.h5'))
