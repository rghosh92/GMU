# -*- coding: utf-8 -*-
"""
Created on Fri Jul 31 19:59:05 2026
...
class Net_vanilla_CNN_convert3(nn.Module):
    def __init__(self,input_channels,layers,kernels,epsilons = [0.0001,0.0001,0.0001],decay_regress=0,decay_errors = 0, classes = 10,use_bn = True,dropping=0,poly_order_init=5):
        ...
        # print(kernels[0])
        self.srn1 = SRNLayer(input_channels, 64, 5,padding=2,epsilon = epsilons[0],num_slices=3,degree=1)
...
#     corruptions = ['general']
    ...
    # net,all_losses = train_network_normal(net,trainloader,testloader,dataset_test.targets, init_rate,total_epoch,decay_normal)
    # torch.save(net.state_dict(), './fmnist_cnn.h5')
    net.load_state_dict(torch.load('./fmnist_cnn.h5'))
    # acc =  test_network(net, testloader)
    # print('Test Accuracy Normal:', acc)
    net1.load_state_dict(torch.load('./fmnist_gmucnn.h5'))

    # net1,all_losses = train_network_normal(net1,trainloader,testloader,dataset_test.targets, init_rate,total_epoch,decay_normal)
    # torch.save(net1.state_dict(), './fmnist_gmucnn.h5')
    # acc =  test_network(net1, testloader)
    # print('Test Accuracy GMU:', acc)
