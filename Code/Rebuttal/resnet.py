...
class ResMLPNet_probing(nn.Module):
    def __init__(self, input_channels, block, num_blocks, num_classes=10):
        ...
        self.layer1 = self._make_layer(block, 128, num_blocks[0])
        self.layer2 = self._make_layer(block, 128, num_blocks[1])

        # Probes (128 → num_classes)
        self.probe1 = nn.Linear(128, num_classes)
        self.probe2 = nn.Linear(128, num_classes)
        self.probe3 = nn.Linear(128, num_classes)
...
class ResGMUMLP_probing(nn.Module):
    def __init__(self, input_channels, block, num_blocks, medians,
                 num_slices=1, degree=1, normalize=True, epsilon=1e-4,
        ...
        # GMU layers
        self.gmu1 = SimpleGMULayer_release(input_channels, 128, epsilon, num_slices, degree, normalize)
        # self.gmu2 = SimpleGMULayer_release(input_channels, 32, epsilon, num_slices, degree, normalize)
        # self.gmu3 = SimpleGMULayer_release(input_channels, 32, epsilon, num_slices, degree, normalize)
        # self.gmu4 = SimpleGMULayer_release(input_channels, 32, epsilon, num_slices, degree, normalize)
...
class ResHMUMLP_probing(nn.Module):
    def __init__(self, input_channels, block, num_blocks, medians,
        ...
        # GMU layers
        self.hmu1 = HMU(d = input_channels, k = num_slices)(input_channels, 128, epsilon, num_slices, degree, normalize)
