...
# 2. Standard hook function to flatten features for the linear probe
def get_features(storage_dict, name):
    def hook(model, input, output):
        # Both will output [Batch, 64, 32, 32] for CIFAR-10. 
        # Flattening gives [Batch, 65536]
        storage_dict[name] = output.flatten(start_dim=1).detach()
    return hook
...
def extract_first_layer(model, dataloader, feature_dict, layer_key='conv1', device='cuda'):
    ...
    with torch.no_grad():
        for inputs, labels in dataloader:
            ...
            all_feats[ptr : ptr + batch_size] = feature_dict[layer_key].cpu().numpy()
...
def run_linear_probe(train_x, train_y, test_x, test_y, layer_name, num_classes=10, epochs=100, lr=0.01, batch_size=100):
...
#     corruptions = ['general']
    ...
    net_gmu = VGG('VGG16-GMU',num_slices=5)
    ...
    net_gmu.load_state_dict(torch.load('./VGG16_SRN5_CIFAR10.h5',weights_only=True))
...
#    
    ...
    # 1. Dictionaries to hold the extracted features
    features_normal = {}
    features_gmu = {}

    # 3. Register hook for VGG16-GMU (Immediately after the GMU layer)
    net_gmu.srn1.register_forward_hook(get_features(features_gmu, 'gmu_out'))

    # 4. Register hook for Normal VGG16 (Immediately after the 1st Conv -> BN -> ReLU)
    # In self.features: [0]=Conv2d, [1]=BatchNorm2d, [2]=ReLU
    net.features[2].register_forward_hook(get_features(features_normal, 'conv1_relu_out'))
    
    
    # Extract features using your dataloaders
    print("Extracting Normal VGG16 features...")
    # train_feat_norm, train_labels = extract_first_layer(net, trainloader, features_normal, 'conv1_relu_out')
    # test_feat_norm, test_labels   = extract_first_layer(net, testloader, features_normal, 'conv1_relu_out')
    
    print("Extracting GMU VGG16 features...")
    train_feat_gmu, _ = extract_first_layer(net_gmu, trainloader, features_gmu, 'gmu_out')
    test_feat_gmu, _  = extract_first_layer(net_gmu, testloader, features_gmu, 'gmu_out')
    
    # print(train_feat_norm.shape)
    print(train_feat_gmu.shape)
    
    # Run Probes
    # run_linear_probe(train_feat_norm, train_labels, test_feat_norm, test_labels, layer_name="VGG16 Normal (Conv1+ReLU)")
    run_linear_probe(train_feat_gmu, train_labels, test_feat_gmu, test_labels, layer_name="VGG16 GMU (srn1)")
