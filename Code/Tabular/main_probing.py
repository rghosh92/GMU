    ...
    for epoch in range(epochs):
        ...
        for i, data in enumerate(trainloader, 0):
            ...
            if hasattr(net, 'gmu1'):
                ...
                # sparsity_loss = torch.norm(net.gmu1.weights, 1)
...
# 3, 14, 16, 18, 22, 23, 24, 28, 30, 36, 38,  57, 60, 
    names = [233, 244, 246, 248, 252, 253, 254, 258, 260, 266, 273, 275, 288, 1767, 1778, 
    ...
    for d in range(len(names)):
        for i in range(trials):
            ...
            # ============================================================
            # BASELINE MODEL (train once per dataset)
            # ============================================================
            ...
            train_acc_base = float(test_network(net_base, trainloader, my_dataset.labels, 0))
            ...
            print("Test:", test_acc_base)

            # ============================================================
            # GMU MODELS FOR num_slices = 0..5
            # ============================================================
            for num_slices in range(4):

                print(f"\n=== Running GMU with num_slices = {num_slices} ===")

                net_gmu = ResGMUMLP_probing(
                    input_channels,
                    BasicBlock,
                    [1, 1],
                    medians,
                ...
                net_gmu.mode = 'regress_batch'

                net_gmu, _ = train_network_normal(
                    net_gmu, trainloader, init_rate, total_epoch,
                    decay_normal, testloader, my_dataset_test
                )

                train_acc_gmu = float(test_network(net_gmu, trainloader, my_dataset.labels, 0))
                test_acc_gmu = test_network(net_gmu, testloader, my_dataset_test.labels, 0)
# 
                # GMU PROBES
                f1, f2, f3, ytrain = collect_probe_features(net_gmu, trainloader)
                gmu_probe1 = train_probe(f1, ytrain, classes)
                gmu_probe2 = train_probe(f2, ytrain, classes)
                gmu_probe3 = train_probe(f3, ytrain, classes)

                print("\nGMU Probe Accuracies:")
                print("After GMU:", gmu_probe1)
                print("After Layer1:", gmu_probe2)
                print("After Layer2:", gmu_probe3)
                print("Test:", test_acc_gmu)
                ...
                results.append({
                    ...
                    "GMU Train Acc": train_acc_gmu,
                    "GMU Test Acc": test_acc_gmu,
                    "GMU Probe1": gmu_probe1,
                    "GMU Probe2": gmu_probe2,
                    "GMU Probe3": gmu_probe3,
    ...
    with open("dataset_results_norelu.json", "w") as f:
...
#             train_acc_gmu = test_network(net_gmu, trainloader, my_dataset.labels, 0)
#             test_acc_gmu = test_network(net_gmu, testloader, my_dataset_test.labels, 0)
